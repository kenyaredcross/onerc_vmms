<template>
	...
	<div v-else-if="alreadyApplied && applicationStatus !== 'Draft'" class="text-center py-10">
		<PendingApproval />
	</div>
...
const jobApplication = createResource({
	...
	onSuccess(data) {
		if (data?.name) {
			...
			applicationStatus.value = application.docstatus == 0 ? "Draft" : application.name;
...
useHead({
	title: "Volunteer Signup | Gambia Red Cross Society",
	meta: [
		{
			name: "description",
			content:
				"Join the Gambia Red Cross Society as a volunteer and make a difference in your community. Sign up to contribute your skills, time, and compassion.",
		},
	],
});
</script>
