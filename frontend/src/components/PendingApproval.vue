		...
		<div class="max-w-6xl w-full grid grid-cols-1 md:grid-cols-2 gap-12 p-10">
			<div class="flex flex-col justify-center space-y-6 text-left">
				<h1 class="text-3xl md:text-6xl font-extrabold text-red-600 leading-tight">
					{{ __("Application") }} <br />
					<span class="text-gray-800">{{ __("Received") }}</span>
				</h1>
				<p class="text-lg text-gray-600 max-w-lg">
					{{ __("Thank you for your interest in joining the") }}
					<span class="font-semibold text-red-500">{{
						__("Tanzania Red Cross Society")
					}}</span
					>. {{ __("Your application is now") }}
					<span class="text-red-500 font-semibold">{{ __("under review") }}</span>
					{{ __("and we'll get back to you soon.") }}
				</p>
			</div>

			<div class="flex flex-col items-center justify-center space-y-6">
				<div class="bg-white rounded-2xl p-8 shadow-lg max-w-sm w-full text-center">
					<div
					...
					<p class="text-gray-600 mb-6">
						{{ __("Our team is currently reviewing your submission.") }}
					</p>
