<template>
	...
	<div
		:class="
			bare
				? ''
				: 'rounded-lg border border-gray-200 bg-white px-4 py-4 shadow-sm transition-colors'
		"
	>
		...
		<!-- State area -->
		<!-- 1) uploading -->
		<div v-if="uploading" class="rounded-md border border-gray-200 bg-gray-50 px-3 py-3">
			<div class="flex items-center justify-between text-sm mb-2">
				<span class="flex items-center gap-2 text-gray-700 min-w-0">
					<Loader2 class="w-4 h-4 text-red-600 animate-spin flex-shrink-0" />
					<span class="truncate min-w-0">{{ uploadingName }}</span>
				</span>
				<span class="text-xs text-gray-500 flex-shrink-0">{{ progress }}%</span>
			</div>
		...
		<div
			v-else-if="hasFile"
			class="rounded-md border border-gray-200 bg-gray-50 px-3 py-2.5 flex items-center gap-3"
		>
			<div
				class="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded bg-white border border-gray-200 text-red-600"
			>
			...
			<div class="flex-1 min-w-0">
				<a
					...
					target="_blank"
					rel="noopener"
					class="block text-sm font-medium text-gray-800 hover:text-red-700 truncate"
				>
					{{ fileName }}
				</a>
				<p class="text-xs text-gray-500 truncate">{{ metaLine }}</p>
			</div>
		...
		<div
			...
			@drop.prevent="onDrop"
		>
			<UploadCloud class="mx-auto h-7 w-7 text-gray-400 pointer-events-none" />
