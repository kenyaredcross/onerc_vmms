<template>
	<div v-if="job.data" class="px-4 sm:px-6 pt-8 max-w-7xl space-y-10 h-fit mb-10">
		<div class="p-8 bg-white rounded-2xl shadow-sm border border-gray-100">
			...
			<div class="flex flex-wrap gap-3 mb-10">
				<Badge size="lg" class="bg-gray-50 text-gray-700 border border-gray-200">
					<template #prefix
						><CalendarDays class="w-4 h-4 stroke-2 text-gray-500"
					/></template>
					{{ __("Posted") }} {{ __(dayjs(job.data.creation).fromNow()) }}
				</Badge>
				...
				<Badge size="lg" class="bg-blue-50 text-blue-700 border border-blue-200">
					<template #prefix
						><Briefcase class="w-4 h-4 stroke-2 text-blue-600"
					/></template>
					{{ __(job.data.designation?.designation_name || job.data.designation?.name) }}
				</Badge>
				...
				>
					<template #prefix><Award class="w-4 h-4 stroke-2 text-green-600" /></template>
					{{ __(job.data.minimum_years_of_experience) }}{{ __("+ Years") }}
				</Badge>
			</div>

			<section v-if="job.data.required_skills?.length" class="mb-10">
				<h2 class="section-title"><CheckCircle class="icon" /> {{ __("Core Skills") }}</h2>
				<div class="section-card flex flex-wrap gap-2">
					<span
						...
						class="skill-badge"
					>
						<Check class="w-3 h-3 text-red-600" /> {{ __(skill?.skill) }}
					</span>
				</div>
			</section>

			<section v-if="hasQualification(job.data)" class="mb-10">
				<h2 class="section-title"><FileText class="icon" /> {{ __("Qualifications") }}</h2>
				<div class="section-card grid sm:grid-cols-2 gap-6">
					<div>
						<p class="section-label">{{ __("Minimum Qualification") }}</p>
						<p class="section-value">
							{{ __(job.data.minimum_qualification_level) }}
						</p>
					</div>
					<div>
						<p class="section-label">{{ __("Field of Study") }}</p>
						<p class="section-value">{{ __(job.data.preferred_field_of_study) }}</p>
					</div>
					<div>
						<p class="section-label">{{ __("Minimum GPA / Grade") }}</p>
						<p class="section-value">{{ __(job.data.required_gpa__grade) }}</p>
					</div>
					<div>
						<p class="section-label">{{ __("Equivalent Experience") }}</p>
						<p class="section-value">
			...
			<section v-if="job.data.required_licences?.length" class="mb-10">
				<h2 class="section-title"><Award class="icon" /> {{ __("Required Licenses") }}</h2>
				<div class="section-card flex flex-wrap gap-2">
					<span
						...
						class="skill-badge"
					>
						<Check class="w-3 h-3 text-red-600" /> {{ __(lic?.license_type) }}
					</span>
				</div>
			</section>
			...
			<section v-if="job.data.description">
				<h2 class="section-title">
					<FileText class="icon" /> {{ __("Opportunity Description") }}
				</h2>
				<div
