<template>
	<div class="border-0">
		...
		<component
			v-if="!readOnly && (multi || !uploadedFiles.length) && customUploadComponent"
...
const validateFiles = (files: File[]): File[] => {
	...
	for (const file of files) {
		...
		if (file.size > props.maxFileSize * 1024 * 1024) {
			toast.error(`File ${file.name} is too large. Maximum size is ${props.maxFileSize}MB`);
			continue;
		}
...
const removeFile = (index: number) => {
	...
	toast.success(`${removedFile.file_name || removedFile.name || "File"} removed successfully.`);
};

const clearAllFiles = () => {
	if (props.readOnly) return;
	if (confirm(`Are you sure you want to remove all ${uploadedFiles.value.length} files?`)) {
		uploadedFiles.value = [];
...
function initFromModelValue() {
	...
	if (props.multi) {
		if (Array.isArray(props.modelValue)) {
			uploadedFiles.value = props.modelValue.map((v: any) => normalizeIncomingFileObj(v));
		}
	} else {
		if (!props.modelValue) {
			uploadedFiles.value = [];
...
function normalizeIncomingFileObj(v: any) {
	...
	if (v && typeof v === "object") {
		return {
			file_name: v.file_name || v.name || (v.file_url || v.url || "").split("/").pop(),
