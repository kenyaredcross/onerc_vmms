...
"""Create the Module Def for VMMS Stipend.

A fifth patch rather than a re-run of the four before it, because a Frappe patch
executes **once per site, by name**. `register_modules`,
`register_member_modules`, `register_volunteer_module` and
`register_deployment_module` are already in this site's Patch Log from the
earlier stages, so adding a module to `modules.txt` afterwards would never create
its Module Def — and every doctype in it fails to import during migrate with a
...
`CLAUDE.md` describes; this is the fifth instance of it.

Runs pre-model-sync, so the module exists before its doctypes are synced.
`add_module_defs` skips modules that already have one, so this is safe on a fresh
install where `modules.txt` was read at install time.
...
in the same migrate can resolve the new module to this app —
`load_doctype_module` falls back to `frappe.core` and every doctype in the module
fails with
