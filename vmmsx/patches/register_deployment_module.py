...
"""Create the Module Def for VMMS Deployment.

A fourth patch rather than a re-run of the three before it, because a Frappe
patch executes **once per site, by name**. `register_modules`,
`register_member_modules` and `register_volunteer_module` are already in this
site's Patch Log from the earlier stages, so adding a module to `modules.txt`
afterwards would never create its Module Def — and every doctype in it fails to
import during migrate with a message about `frappe.core.doctype.<name>` not
existing, which says nothing at all about the real cause.

So each time a module is added, a patch is added with it. That is the pattern
`CLAUDE.md` describes; this is the fourth instance of it.
