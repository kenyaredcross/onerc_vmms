...
class VMNotificationCenter(Document):
	...
	if TYPE_CHECKING:
		...
		email_message: DF.TextEditor | None
		expired: DF.Check
		...
		sms_message: DF.SmallText | None
		title: DF.Data
		total_recipients: DF.Int
	# end: auto-generated types

	def validate_sms_size(self):
		if self.communication_channel == "SMS" and self.sms_message:
			max_sms_length = 160
			if len(self.sms_message) > max_sms_length:
				frappe.throw(
					f"SMS message cannot exceed {max_sms_length} characters. Currently {len(self.sms_message)} characters."
				)

	def on_submit(self):
		self.queue_action("send_notification")
		msgprint(
			msg="Notification has been queued to be sent",
	...
	def before_submit(self):
		validation_rules = {
			"SMS": ("sms_message", "SMS message"),
			"Email": ("email_message", "Email message"),
		}
		field, label = validation_rules.get(self.communication_channel, (None, None))
		if field and not getattr(self, field):
			frappe.throw(f"Please enter the {label} before submitting.")

		self.validate_sms_size()
	...
	def _send_sms(self):
		...
		core_send_sms(receiver_list=recipient_nos, msg=cstr(self.sms_message))

	def _send_email(self):
		self.get_email_settings()
		recipient_list = self.get_recipient_list()
		self.db_set("total_recipients", len(recipient_list), update_modified=False)
		self.reload()
	...
	def _fetch_personnel_recipients(self) -> list[dict[str, str]]:
		...
		conditions = self._build_conditions(recipient, self.map_personel_filters_to_registry())
	...
	def _fetch_member_recipients(self) -> list[dict[str, str]]:
		...
		member_conditions = self._build_conditions(recipient, self.map_personel_filters_to_registry())
		membership_conditions = self._build_conditions(membership, self.map_membership_filters_to_registry())
	...
	def _build_conditions(table: DocType, filters: dict[str, list]) -> list:
		return [table[field].isin(values) for field, values in filters.items() if values]
	...
	def _map_filters_to_registry(self, filter_definitions: list[dict[str, str]]) -> dict[str, list]:
		...
		for filter_def in filter_definitions:
			for self_field, doctype_field in filter_def.items():
				...
				if rows:
					result[doctype_field] = [getattr(row, doctype_field) for row in rows]
		return result
	...
	def get_sms_settings():
		if not frappe.db.get_single_value("SMS Settings", "sms_gateway_url", cache=True):
			frappe.throw(
	...
	def get_email_attachments(self) -> list[dict[str, str]]:
		return [
			{"fname": att.get("file_name"), "file_url": att.get("file_url")}
			for att in get_attachments(self.doctype, self.name)
		]
