...
class PersonnelDataImport(Document):
    ...
    def get_preview_from_template(self):
        ...
        valid_indices = [c["index"] for c in importer.columns if not (not c["header_title"] and c["skip_import"])]
        ...
        return {
            "columns": [{"header_title": _("Sr. No")}] + [c for c in importer.columns if c["index"] in valid_indices],
            "data": preview_data,
...
def execute_import_chunk(data_import_name, start_index, end_index):
    ...
    counts = frappe.db.get_value("Personnel Data Import", data_import_name, ["successes", "failures"], as_dict=1)
    frappe.db.set_value("Personnel Data Import", data_import_name, {
