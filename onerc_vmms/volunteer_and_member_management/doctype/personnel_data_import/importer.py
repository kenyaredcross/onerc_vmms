...
class PersonnelDataImporter:
    ...
    def get_field_def(self, doctype, fieldname):
        if "." in fieldname:
            ...
            if df:
                return {"fieldtype": df.fieldtype, "fieldname": fieldname, "label": df.label, "options": df.options, "parent": doctype}
    ...
    def auto_match(self, header_title):
        search_doctypes = []
        if self.import_mode in ["Employee", "Both"]: search_doctypes.append(self.employee_doctype)
        if self.import_mode in ["User", "Both"]: search_doctypes.append(self.user_doctype)
    ...
    def validate_and_transform(self, doctype, fieldname, value, is_child=False):
        if value in INVALID_VALUES: return None
        meta = frappe.get_meta(doctype)
        df = meta.get_field(fieldname)
        if not df: return value
        ...
        if df.fieldtype == "Link":
            ...
            if title_field:
                existing_by_title = frappe.db.get_value(target_doctype, {title_field: ["like", value]}, "name")
    ...
    def import_single_row(self, row):
        try:
            ...
            if self.import_mode in ["Employee", "Both"]:
                ...
                if e_base:
                    if user_id: e_base["user_id"] = user_id
