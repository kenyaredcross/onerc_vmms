...
def get_interviewers():
	...
	users_with_roles = frappe.get_all(
		...
		filters={
			"name": [
				"in",
				frappe.get_all("Has Role", filters={"role": ["in", allowed_roles]}, pluck="parent"),
			],
			"enabled": 1,
		},
		pluck="name",
...
@frappe.whitelist()
def update_meta_info(type: str, route: str, meta_tags: list):
	frappe.has_permission("Website Route Meta", "write", throw=True)
...
def set_field_value(doc, fieldname, value, fieldtype=None):
	...
	if fieldtype == "Table MultiSelect":
		...
		if not isinstance(value, list):
			frappe.throw(f"Expected list of values for Table MultiSelect field {fieldname}")
			...
			if isinstance(item, str):
				...
				if row_data.get("name") and row_data["name"] in existing:
					...
					if link_field not in row_data:
						frappe.throw(f"Missing link field {link_field} for new child row in {fieldname}")
					doc.append(fieldname, row_data)

			else:
				frappe.throw(f"Unsupported item type {type(item)} for Table MultiSelect field {fieldname}")

	elif fieldtype == "Table":
		if isinstance(value, list):
			doc.set(fieldname, [])
			child_meta = frappe.get_meta(frappe.get_meta(doc.doctype).get_field(fieldname).options)
			for row in value:
				if isinstance(row, dict):
					...
					for k, v in row.items():
						...
						if df.fieldtype in ("Attach", "Attach Image"):
							...
							processed_row[k] = v
					has_values = any(val not in (None, "", []) for val in processed_row.values())
					if has_values:
		...
		if isinstance(value, dict) and value.get("file_url"):
			...
			frappe.throw(f"Expected file url (string) or object with file_url for field {fieldname}")
...
def create_or_update_skill_map(employee: str, skills: str | list | dict):
	...
	from ..api.volunteer import validate_volunteer
