...
def on_update(doc: Document, method: str) -> None:
	...
	existing_employee_skills = {row.skill for row in getattr(employee_doc, "skills", [])}
	...
	skill_maps = frappe.get_all("Employee Skill Map", filters={"employee": employee}, fields=["name"])

	if not skill_maps:
		skill_map_doc = frappe.get_doc({"doctype": "Employee Skill Map", "employee": employee})
