...
def send_opportunity_applicant_rejections() -> None:
    try:
        ...
        for opening_data in closed_openings:
            ...
            closing_date = getattr(opening, "closed_on", None) or getattr(
                opening, "closes_on", None
            )
            shortlisted_rejection_notification_date = config.get(
                "shortlisted_rejection_notification_date"
            )
            notify_unshortlisted_applicants_after = config.get(
            ...
            if (today_date == notify_unshortlisted_applicants_date) or (
                ...
                applicants: List[Dict[str, Any]] = get_rejected_applicants(
                ...
                send_rejection_emails(
                    applicants, template_name=config["rejection_email_template"]
                )

    except Exception:
        frappe.log_error(
            "Job Applicant -> Send Rejections Error", frappe.get_traceback()
        )
        frappe.throw("An error occurred while sending rejection notifications.")


def is_auto_rejection_enabled(opening: Optional[Document]) -> bool:
...
def get_rejection_config(
    opening: Optional[Document], settings: Document
) -> Dict[str, Any]:
...
def get_rejected_applicants(
    job_opening: Optional[str] = None, job_applicant: Optional[str] = None
) -> List[Dict[str, Any]]:
...
@frappe.whitelist()
def send_rejection_emails(applicants: Any, template_name: Optional[str] = None) -> None:
    """
...
@frappe.whitelist()
def send_rejection_email(name: str, template_name: Optional[str] = None) -> bool:
