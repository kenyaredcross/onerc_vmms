	...
	try:
		...
		for resp in responses:
			...
			if question.is_required and not resp.answer:
				...
				score = min(max_score, question.weight) if resp.answer == question.expected_answer else 0
		...
		if knock_off_failed:
			...
			try:
				min_pass = float(minimum_pass_score) if minimum_pass_score is not None else None
			...
			if score is not None and min_pass is not None:
				doc.eligibility_status = "Eligible" if score >= min_pass else "Pending Review"
			else:
